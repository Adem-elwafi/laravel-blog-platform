          ...
          <li key={friend.id} className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <div className="w-10 h-10 bg-gradient-to-br from-brand-900 to-brand-700 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                  {friend.name ? friend.name.charAt(0).toUpperCase() : '?'}
                </div>
                {friend.is_online && (
                  <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-accent border-2 border-white dark:border-gray-800 rounded-full" />
                )}
              </div>
